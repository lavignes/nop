// SDL scancode -> PS/2 Scan Code Set 2 translation.

#include <SDL3/SDL.h>

#include "vm.h"

enum { PS2_CLK_DIV = CPU_HZ / 20000 };
...
static Ps2Code const PS2_TBL[SDL_SCANCODE_COUNT] = {
    [SDL_SCANCODE_A] = {0x1C},
    [SDL_SCANCODE_B] = {0x32},
    [SDL_SCANCODE_C] = {0x21},
    [SDL_SCANCODE_D] = {0x23},
    ...
    [SDL_SCANCODE_RCTRL] = {0x14, TRUE},
    [SDL_SCANCODE_RALT] = {0x11, TRUE},
    [SDL_SCANCODE_LGUI] = {0x1F, TRUE},
    [SDL_SCANCODE_RGUI] = {0x27, TRUE},
    [SDL_SCANCODE_APPLICATION] = {0x2F, TRUE},
    ...
    [SDL_SCANCODE_KP_DIVIDE] = {0x4A, TRUE},
    [SDL_SCANCODE_KP_ENTER] = {0x5A, TRUE},
};

static void ps2Push(Ps2 *ps2, U8 b) {
  U8 next = (ps2->tail + 1) % sizeof(ps2->buf);
  if (next == ps2->head) {
    return;
  }
  ps2->buf[ps2->tail] = b;
...
U8 ps2Dequeue(Ps2 *ps2) {
  if (ps2->head == ps2->tail) {
    return 0;
  }
  U8 byte = ps2->buf[ps2->head];
  ps2->head = (ps2->head + 1) % sizeof(ps2->buf);
  return byte;
}

void ps2Tick(Ps2 *ps2, Via *via, ViaPort port) {
  if (++ps2->clkDiv < PS2_CLK_DIV) {
    return;
  }
  ps2->clkDiv = 0;

  if (!ps2->pulsed && ps2->head != ps2->tail) {
    ps2->pulseCycle = 0;
    ps2->pulsed = TRUE;
  }

  if (ps2->pulsed) {
    if (ps2->pulseCycle < 10) {
      if (ps2->pulseCycle == 0) {
        U8 byte = ps2->buf[ps2->head];
        viaSetPort(via, port, byte);
      }
      viaSetC1(via, port, FALSE);
      ps2->pulseCycle++;
    } else {
...
void ps2Key(Ps2 *ps2, UInt scancode, Bool down) {
  ...
  Ps2Code entry = PS2_TBL[scancode];
  if (entry.code == 0) {
    return;
  }
  if (entry.ext) {
