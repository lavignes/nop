...
SD_CS   = $01 ; PB0, active low
SD_SCK  = $02 ; PB1
SD_MOSI = $04 ; PB2
SD_MISO = $80 ; PB7, input
...
@z:
    sta SDB
    sta VIA_ORB ; drive MOSI, SCK low
    ora #SD_SCK
    sta VIA_ORB ; rising edge: card samples MOSI, presents MISO
    lda VIA_ORB
...
; -------- VIA init --------
ViaInit:
    lda #$00 ; port A: keyboard input
    sta VIA_DDRA
    lda #(SD_CS | SD_SCK | SD_MOSI) ; port B: SPI outputs, PB7 (MISO) input
    sta VIA_DDRB
    lda #SD_CS ; deselect card
    sta VIA_ORB
    sta SD_OUT
    lda #$01 ; port A input latch
    sta VIA_ACR
    lda #$00 ; CA1 negative edge
    sta VIA_PCR
    lda #$7F
    sta VIA_IFR
    lda #$82 ; enable CA1 interrupt (keyboard byte ready)
    sta VIA_IER
    rts
...
Irq:
    ...
    tya
    pha
    lda VIA_ORA ; scancode; reading clears the CA1 flag
    cmp #$F0 ; break prefix
    beq @setrel
